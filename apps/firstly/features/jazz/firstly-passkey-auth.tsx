...
const APP_NAME = "Firstly"

function isFirstlyAuthRoute(pathname: string): boolean {
  return pathname === "/sign-in" || pathname === "/sign-up"
}

function isFirstlyAppRoute(pathname: string): boolean {
  return (
...
export function FirstlyPasskeyAuth({
  children,
}: {
  ...
  useEffect(() => {
    ...
    if (!isFirstlyAuthRoute(pathname)) return
    router.replace("/sessions")
  }, [auth.state, pathname, router])

  useEffect(() => {
    if (auth.state !== "anonymous") return
    if (!isFirstlyAppRoute(pathname)) return
    router.replace("/sign-in")
  }, [auth.state, pathname, router])

  if (auth.state === "signedIn") {
    if (isFirstlyAuthRoute(pathname)) {
      return <AuthRedirectLoading />
    }
    return <>{children}</>
  }

  if (isPublicLanding(pathname) || isFirstlyAuthRoute(pathname)) {
    if (isPublicLanding(pathname)) {
      return <>{children}</>
    }
  } else if (isFirstlyAppRoute(pathname)) {
    return <AuthRedirectLoading />
  }
      ...
      <div className="w-full max-w-sm space-y-6">
        <div className="space-y-1 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            {isSignUpRoute
              ? "Start building skill graphs and structured lessons."
        ...
        {isSignUpRoute ? (
          ...
          >
            <div className="flex flex-col gap-1.5">
              <label htmlFor="firstly-display-name" className="text-sm font-medium">
                Display name
              </label>
              <Input
                id="firstly-display-name"
                name="displayName"
                type="text"
                required
                autoComplete="name"
