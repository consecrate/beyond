...
function SkillTreeIcon({ className }: { className?: string }) {
  return (
    <svg
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogTrigger
        render={
          <Button
            ...
            size="sm"
            className={cn(
              "h-7 shrink-0 gap-1.5 px-2 text-primary hover:bg-primary/10 hover:text-primary",
              "focus-visible:ring-2 focus-visible:ring-ring",
              className,
            )}
            aria-label="Import skill tree JSON"
          >
            <SkillTreeIcon />
            Skill Tree
          </Button>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Paste JSON with <code className="text-xs">lessons</code> and{" "}
            <code className="text-xs">edges</code> (see AI import schema). This replaces
